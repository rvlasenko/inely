...
На этом всё. После этих действий приложение будет доступно по адресу http://madeasy.local на базе сервера Apache 2.4 и MySQL 5.6. Подключиться к базе можно через /adminer или MySQLWorkbench.

Обычно возникает необходимость проброса портов на MySQL. Если вы не смогли подключиться к базе, то зайдите через ssh ```vagrant ssh``` выполните команду ```sudo nano /etc/mysql/my.cnf``` и закомментируйте строки:
```bash
