    ...
    var createProjectNode = function () {
        ...
        if (projName.length) {
            ...
            $.post('/project/create', {
                ...
                // Скрыть оповещение об отсутствии проектов для новых юзеров
                $('.empty').remove();
                $('.jstree-neutron').append(
    ...
    var fillTree = function () {
        $(document).on('click', '.jstree-neutron a', function () {
            ...
            $dropDownMenu.prepend(
                '<li>' +
                    '<a href="#" id="settings">' +
                        '<span class="entypo-user-add margin-iconic"></span>Настройки проекта' +
                    '</a>' +
                '</li>' +
                '<li class="divider"></li>'
            );
